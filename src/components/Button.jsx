function Button({ children, className = "", animation = true, isBig = true }) {
  return (
    <button
      className={`group/button overflow-hidden tracking-(--t-16) px-8 py-5 text-(length:--text-16) h-full w-full flex items-center justify-center text-black text-sm font-bold bg-white rounded-[200px] ${className}`}>
      <div className={`${isBig ? "h-60" : "h-50"}`}>
        <div
